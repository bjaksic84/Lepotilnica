...
import { BUSINESS, SOCIAL_LINKS, OPENING_HOURS } from "@/lib/site";
...
// Derived from the same OPENING_HOURS the structured data uses, so the visible
// hours and the schema.org hours can't drift apart.
const HOURS = [
    ...OPENING_HOURS.map((h) => ({
        day: h.label,
        time: `${h.opens.replace(/^0/, "")} — ${h.closes}`,
        closed: false,
    })),
        ...
        <footer className="bg-charcoal text-porcelain/50 relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 py-16 max-w-5xl flex flex-col items-center text-center">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-12 w-full text-center">
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <ul className="space-y-3 text-sm">
                            <li>{BUSINESS.street}<br />{BUSINESS.postalCode} {BUSINESS.city}</li>
                            <li><a href={`tel:${BUSINESS.phone}`} className="hover:text-gold transition-colors">{BUSINESS.phoneDisplay}</a></li>
                            <li><a href={`mailto:${BUSINESS.email}`} className="hover:text-gold transition-colors">{BUSINESS.email}</a></li>
                        </ul>
                    </div>
                ...
                <div className="flex items-center justify-center gap-3 mt-12">
                    <a href={SOCIAL_LINKS.instagram} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-porcelain/5 border border-porcelain/10 flex items-center justify-center hover:bg-gold/20 hover:border-gold/30 transition-all group" aria-label="Lepotilnica by Karin na Instagramu">
                    ...
                    <a href={SOCIAL_LINKS.facebook} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-porcelain/5 border border-porcelain/10 flex items-center justify-center hover:bg-gold/20 hover:border-gold/30 transition-all group" aria-label="Lepotilnica by Karin na Facebooku">
